...
/** Декодирование тела ответа в одну UTF-8 строку (без поchunk-склейки строкой). */
async function readResponseBodyAsUtf8(res) {
  const buf = new Uint8Array(await res.arrayBuffer());
  return new TextDecoder('utf-8', { fatal: false }).decode(buf);
}

/**
 * OpenAI-совместимые провайдеры иногда отдают content строкой или массивом блоков.
 * @param {unknown} content
 * @returns {string}
 */
export function assistantMessageContentToString(content) {
...
export async function postChatCompletion(opts) {
  ...
  const text = assistantMessageContentToString(data?.choices?.[0]?.message?.content).trim();
  if (!text) {
    const label = provider === 'polza' ? 'Polza' : 'OpenRouter';
