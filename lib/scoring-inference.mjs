...
/**
 * Совпадение базового / допустимого города кандидата с текстом вакансии (нижний регистр).
 * Возвращает 0 или «лучший» балл матча как в прежней логике inferLocationScore: 100 / 98 / 96 / 94.
 * @param {string} hayLower
 * @param {object} prefs
 */
function homeCityPresenceScore(hayLower, prefs) {
  ...
  let best = 0;
  if (base) {
    if (hay.includes(base)) best = Math.max(best, 100);
...
/**
 * В тексте вакансии явно фигурирует город из scoringGeo.baseCity / acceptableCities (с алиасами МСК/СПб).
 * @param {string} hayLower — результат buildScoringBlob (уже lower-case)
 * @param {object} prefs
 */
export function candidateHomeCityAppearsInVacancyBlob(hayLower, prefs) {
  return homeCityPresenceScore(hayLower, prefs) > 0;
}
...
export function inferLocationScore(parsed, prefs) {
  ...
  let best = 38;
  const home = homeCityPresenceScore(hay, prefs);
  if (home > 0) best = home;
