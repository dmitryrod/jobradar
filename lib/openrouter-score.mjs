...
`;
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    const instructionFitRisk = stripLoneUtf16Surrogates(String(parsed.instructionFitRisk || '').trim());

    const mergedRisks = stripLoneUtf16Surrogates(
      mergeInstructionIntoRisks(
        stripLoneUtf16Surrogates(String(parsed.risks || '').trim()),
        instructionFitRisk,
        instructionComplexity
      )
    );

    const summary = stripLoneUtf16Surrogates(String(parsed.summary || '').trim());
