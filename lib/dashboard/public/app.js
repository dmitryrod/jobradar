...
/** Ключи localStorage для пагинации списка открытых ссылок. */
const LS_HARVEST_URLS_PAGE_SIZE = 'harvest_urls_page_size';
const LS_HARVEST_URLS_PAGE = 'harvest_urls_page';

/** @type {'queue' | 'collect' | 'dashboard'} */
let viewMode = 'queue';
let currentStatus = 'pending';

/** Маппинг URL-путей к состоянию приложения (SPA роутинг) */
const ROUTES = {
  '/': { viewMode: 'dashboard' },
  '/find': { viewMode: 'collect' },
  '/check': { viewMode: 'queue', currentStatus: 'pending' },
  '/answer': { viewMode: 'queue', currentStatus: 'approved' },
  '/reject': { viewMode: 'queue', currentStatus: 'rejected' }
};

/** Маппинг состояния обратно в URL-путь */
const ROUTE_TO_PATH = {
  'dashboard': '/',
  'collect': '/find',
  'pending': '/check',
...
/** Состояние пагинации списка открытых ссылок */
let harvestUrlsPagination = {
  pageSize: 100,
  currentPage: 1,
  totalItems: 0,
  allOutcomes: [], // Полный список для пагинации
};
...
/** Применить роут из URL (вызывается при загрузке и popstate) */
function applyRouteFromUrl() {
  const pathname = window.location.pathname;
  const route = ROUTES[pathname];

  if (!route) {
    // Неизвестный путь — редирект на /check (На проверке)
    if (pathname !== '/check') {
    ...
    viewMode = /** @type {any} */ (route.viewMode);
    if (route.currentStatus) {
      currentStatus = route.currentStatus;
    }
  }

  // Синхронизировать UI табов
  syncVacancyTabs();

  // Показать/скрыть панели
  if (harvestPanelEl) harvestPanelEl.hidden = viewMode !== 'collect';
  if (reviewPendingPanelEl) reviewPendingPanelEl.hidden = !(viewMode === 'queue' && currentStatus === 'pending');

  // Загрузить данные
  load();
}

/** Обновить URL из текущего состояния (вызывается при клике на таб) */
function updateRouteFromState() {
  const stateKey = viewMode === 'queue' ? currentStatus : viewMode;
  const newPath = ROUTE_TO_PATH[stateKey];
...
/** Обработчик изменения истории (Назад/Вперёд) */
...
function openDraftModal(item) {
  ...
  btnRefresh.className = 'btn-icon btn-refresh-draft';
  btnRefresh.title = 'Сгенерировать новый черновик';
  btnRefresh.setAttribute('aria-label', 'Сгенерировать новый черновик');
  ...
  btnRefresh.addEventListener('click', async () => {
    ...
    showToast('Генерация нового черновика…', 'neutral');
    try {
      const res = await requestCoverLetterGenerate(item.id, true);
      const newVariants = res.coverLetter?.variants || [];
      if (!newVariants.length) {
        showToast('Не удалось сгенерировать новый черновик', 'bad');
        return;
      }
      draftModalState.variants = newVariants.map((s) => String(s));
      draftModalState.selectedIndex = 0;
      ta.value = draftModalState.variants[0];
      // Обновить radio-кнопки вариантов
      fieldset.querySelectorAll('input[type="radio"]').forEach((radio, i) => {
        radio.checked = i === 0;
      });
      showToast('Новый черновик готов', 'good');
    } catch (e) {
      if (e.status === 409) {
        showToast('Уже есть утверждённое письмо. Сначала отклоните его.', 'bad');
      } else {
...
function renderCard(item) {
  ...
  if (item.status === 'pending') {
    ...
    // Скрыть кнопку "Создать сопроводительное", если черновик уже создан
    if (coverBtn) {
      coverBtn.hidden = hasLetterDraft;
    }
...
function syncVacancyTabs() {
  ...
  vacancyTabsEl.querySelectorAll('.tab').forEach((b) => {
    if (b.dataset.tab === 'dashboard') {
      b.classList.toggle('active', viewMode === 'dashboard');
    } else if (b.dataset.tab === 'collect') {
...
if (harvestPanelEl) {
  ...
  harvestPanelEl.querySelector('[data-ra="reviewAutomationEnabled"]')?.addEventListener('change', () => syncReviewAutomationEnabled(harvestPanelEl));

  // Инициализация пагинации списка открытых ссылок
  loadHarvestUrlsPaginationState();

  // Обработчик селектора размера страницы
  harvestPanelEl.querySelector('.harvest-urls-page-size')?.addEventListener('change', (e) => {
    const newSize = Number(e.target.value);
    setHarvestUrlsPageSize(newSize);
  });

  // Обработчики кнопок пагинации (верх и низ)
  harvestPanelEl.querySelectorAll('.harvest-urls-pagination').forEach((pagination) => {
    const prevBtn = pagination.querySelector('.btn-pagination-prev');
    const nextBtn = pagination.querySelector('.btn-pagination-next');

    prevBtn?.addEventListener('click', () => {
      goToHarvestUrlsPage(harvestUrlsPagination.currentPage - 1);
    });
...
async function refreshVacancyCounts() {
  ...
  try {
    ...
    setQueueTabCount('pending', pending);
    setQueueTabCount('approved', approved);
    setQueueTabCount('rejected', rejected);
    
    // Обновить счетчик Главной вкладки (общий)
    const dashboardEl = vacancyTabsEl.querySelector('[data-tab-count="dashboard"]');
    if (dashboardEl) dashboardEl.textContent = String(total);
  } catch {
    /* ignore */
  }
}
...
/**
 * Управляет disabled состоянием полей автоматизации в зависимости от master-чекбокса.
 * Работает только для панели "Сбор" — на "На проверке" toggle нет и поля всегда активны.
 * @param {HTMLElement | null} container — #harvest-panel или #review-pending-panel
 */
function syncReviewAutomationEnabled(container) {
  if (!container) return;
  const enabledCheckbox = container.querySelector('[data-ra="reviewAutomationEnabled"]');
  // Если нет toggle — это панель "На проверке", там поля всегда активны
  if (!enabledCheckbox) return;

  const isEnabled = !!enabledCheckbox.checked;

  // Для панели "Сбор" (.review-automation-collect)
  const parentCollect = container.querySelector('.review-automation-collect');
  if (parentCollect) {
    parentCollect.classList.toggle('disabled', !isEnabled);
    const fieldsContainer = parentCollect.querySelector('.review-automation-fields');
    const saveBtn = parentCollect.querySelector('.btn-review-automation-save');
...
function applyReviewAutomationToContainer(container, ra, opts = {}) {
  ...
  // Устанавливаем master-чекбокс enabled (по умолчанию true для обратной совместимости)
  const enabledCheckbox = container.querySelector('[data-ra="reviewAutomationEnabled"]');
  if (enabledCheckbox) {
    enabledCheckbox.checked = ra.enabled !== false; // undefined или true = включено
  }
  ...
  // Применяем disabled состояние после установки всех значений
  syncReviewAutomationEnabled(container);
...
/** Загрузить сохранённые настройки пагинации из localStorage */
function loadHarvestUrlsPaginationState() {
  const savedPageSize = localStorage.getItem(LS_HARVEST_URLS_PAGE_SIZE);
  const savedPage = localStorage.getItem(LS_HARVEST_URLS_PAGE);

  if (savedPageSize) {
    const size = Number(savedPageSize);
    if ([100, 200, 300, 400, 500, 1000].includes(size)) {
      harvestUrlsPagination.pageSize = size;
    }
  }
  if (savedPage) {
    const page = Number(savedPage);
    if (page >= 1) {
      harvestUrlsPagination.currentPage = page;
    }
  }

  // Установить начальное значение в селекторе
  const pageSizeSelect = harvestPanelEl?.querySelector('.harvest-urls-page-size');
  if (pageSizeSelect) {
    pageSizeSelect.value = String(harvestUrlsPagination.pageSize);
  }
}

/** Сохранить настройки пагинации в localStorage */
function saveHarvestUrlsPaginationState() {
  localStorage.setItem(LS_HARVEST_URLS_PAGE_SIZE, String(harvestUrlsPagination.pageSize));
  localStorage.setItem(LS_HARVEST_URLS_PAGE, String(harvestUrlsPagination.currentPage));
}

/** Отрендерить строку таблицы */
function renderHarvestUrlRow(row) {
  const tr = document.createElement('tr');
  ...
  tr.append(tdAt, tdLink, tdTitle, tdAct);
  return tr;
}

/** Отрендерить строку для старого формата (только URL) */
function renderHarvestUrlRowLegacy(url) {
  const tr = document.createElement('tr');
  const tdLink = document.createElement('td');
  tdLink.className = 'harvest-urls-col-link';
  const a = document.createElement('a');
...
/** Обновить UI пагинации */
function updateHarvestUrlsPaginationUI() {
  if (!harvestPanelEl) return;

  const stats = harvestPanelEl.querySelector('.harvest-stats');
  if (!stats) return;

  const { pageSize, currentPage, totalItems } = harvestUrlsPagination;
  const totalPages = Math.max(1, Math.ceil(totalItems / pageSize));

  // Обновить счётчик
  const countEl = stats.querySelector('.harvest-urls-count');
  if (countEl) countEl.textContent = String(totalItems);

  // Обновить пагинацию (верх и низ)
  const paginations = stats.querySelectorAll('.harvest-urls-pagination');
  paginations.forEach((pagination) => {
    const currentEl = pagination.querySelector('.pagination-current');
    const totalEl = pagination.querySelector('.pagination-total');
    const prevBtn = pagination.querySelector('.btn-pagination-prev');
    const nextBtn = pagination.querySelector('.btn-pagination-next');
...
/** Отрендерить текущую страницу таблицы */
function renderHarvestUrlsTable() {
  if (!harvestPanelEl) return;

  const tbody = harvestPanelEl.querySelector('.harvest-urls-tbody');
  if (!tbody) return;

  tbody.innerHTML = '';

  const { pageSize, currentPage, allOutcomes } = harvestUrlsPagination;
  ...
  const endIndex = Math.min(startIndex + pageSize, allOutcomes.length);

  // Отрендерить только текущую страницу
  for (let i = startIndex; i < endIndex; i++) {
    const row = allOutcomes[i];
    tbody.appendChild(renderHarvestUrlRow(row));
  }
}

/** Перейти на страницу пагинации */
function goToHarvestUrlsPage(page) {
  const { pageSize, totalItems } = harvestUrlsPagination;
  const totalPages = Math.max(1, Math.ceil(totalItems / pageSize));

  // Ограничить страницу допустимыми границами
  let newPage = Math.max(1, Math.min(page, totalPages));

  harvestUrlsPagination.currentPage = newPage;
  saveHarvestUrlsPaginationState();
  renderHarvestUrlsTable();
  updateHarvestUrlsPaginationUI();
}

/** Изменить размер страницы */
function setHarvestUrlsPageSize(newSize) {
  const validSizes = [100, 200, 300, 400, 500, 1000];
  const size = validSizes.includes(newSize) ? newSize : 100;

  harvestUrlsPagination.pageSize = size;

  // Пересчитать текущую страницу чтобы не выйти за границы
  const { totalItems } = harvestUrlsPagination;
  const totalPages = Math.max(1, Math.ceil(totalItems / size));
  harvestUrlsPagination.currentPage = Math.min(harvestUrlsPagination.currentPage, totalPages);
...
async function refreshHarvestStats() {
  ...
  // Обновить отдельные значения для ratio-отображения
  const openedEl = stats.querySelector('.harvest-stat-opened');
  const queuedEl = stats.querySelector('.harvest-stat-queued');
  if (openedEl) openedEl.textContent = String(s.uniqueUrlsOpened ?? 0);
  if (queuedEl) queuedEl.textContent = String(s.uniqueUrlsQueued ?? 0);

  // Подготовить данные для пагинации
  const outcomes = Array.isArray(s.urlOutcomes) && s.urlOutcomes.length > 0 ? s.urlOutcomes : null;
  const legacyUrls = !outcomes && Array.isArray(s.urlsOpened) ? s.urlsOpened : [];

  // Преобразовать в единый формат
  if (outcomes) {
    harvestUrlsPagination.allOutcomes = outcomes;
  } else if (legacyUrls.length > 0) {
    harvestUrlsPagination.allOutcomes = legacyUrls.map(url => ({ url, title: '—', at: null, outcome: 'unknown' }));
  } else {
    harvestUrlsPagination.allOutcomes = [];
  }
  harvestUrlsPagination.totalItems = harvestUrlsPagination.allOutcomes.length;

  // Отрендерить таблицу с пагинацией
  renderHarvestUrlsTable();
  updateHarvestUrlsPaginationUI();
...
/** Отрендерить главную страницу с метриками дашборда */
async function renderDashboardHome() {
  if (!listEl) return;

  // Загрузка статистики
  const [pendingRes, approvedRes, rejectedRes, harvestStatus] = await Promise.all([
    api('/api/vacancies?status=pending').catch(() => ({ items: [] })),
    api('/api/vacancies?status=approved').catch(() => ({ items: [] })),
    api('/api/vacancies?status=rejected').catch(() => ({ items: [] })),
    api('/api/harvest-status').catch(() => ({ running: false }))
  ]);

  const pendingCount = pendingRes.items?.length || 0;
  const approvedCount = approvedRes.items?.length || 0;
  const rejectedCount = rejectedRes.items?.length || 0;
  const totalCount = pendingCount + approvedCount + rejectedCount;

  // Форматирование даты последнего запуска
  let lastActivityText = 'Нет данных';
  if (harvestStatus.exitAt) {
    const d = new Date(harvestStatus.exitAt);
    if (!Number.isNaN(d.getTime())) {
      lastActivityText = d.toLocaleString('ru-RU', { dateStyle: 'short', timeStyle: 'short' });
    }
  }
  ...
  listEl.innerHTML = `
    <div class="dashboard-home">
      <h2 class="dashboard-title">Обзор вакансий</h2>
      <div class="dashboard-stats-grid">
        <div class="stat-card stat-card-total">
          <h3>Всего в очереди</h3>
          <div class="stat-value">${totalCount}</div>
        </div>
        <div class="stat-card stat-card-pending">
          <h3>На проверке</h3>
          <div class="stat-value">${pendingCount}</div>
          <a href="/check" class="stat-link" data-nav="check">Открыть →</a>
        </div>
        <div class="stat-card stat-card-approved">
          <h3>Подходят</h3>
          <div class="stat-value">${approvedCount}</div>
          <a href="/answer" class="stat-link" data-nav="answer">Открыть →</a>
        </div>
        <div class="stat-card stat-card-rejected">
          <h3>Отклонённые</h3>
          <div class="stat-value">${rejectedCount}</div>
          <a href="/reject" class="stat-link" data-nav="reject">Открыть →</a>
        </div>
      </div>
      ...
      <div class="dashboard-section">
        ...
        <div class="dashboard-status">
          <span class="status-indicator ${harvestStatus.running ? 'status-active' : 'status-inactive'}"></span>
          <span class="status-text">
            ${harvestStatus.running ? 'Идёт сбор вакансий' : 'Сбор не активен'}
          </span>
          ${harvestStatus.uniqueUrlsOpened > 0 ? `<span class="status-detail">Открыто URL: ${harvestStatus.uniqueUrlsOpened}</span>` : ''}
        </div>
        ${harvestStatus.running && harvestStatus.startedAt ? `<div class="status-meta">Запущен: ${new Date(harvestStatus.startedAt).toLocaleString('ru-RU')}</div>` : ''}
        ${!harvestStatus.running && lastActivityText !== 'Нет данных' ? `<div class="status-meta">Последняя активность: ${lastActivityText}</div>` : ''}
      </div>

      <div class="dashboard-section">
        <h3>Быстрые действия</h3>
        <div class="dashboard-actions">
          <a href="/find" class="btn btn-primary" data-nav="find">Перейти к сбору</a>
          <a href="/check" class="btn" data-nav="check">Проверить вакансии</a>
        </div>
      </div>
    </div>
  `;

  // Привязать обработчики кликов для навигации
  listEl.querySelectorAll('a[data-nav]').forEach((link) => {
  ...
  vacancyTabsEl.querySelectorAll('.tab').forEach((btn) => {
    btn.addEventListener('click', () => {
      if (btn.dataset.tab === 'dashboard') {
        ...
        if (reviewPendingPanelEl) reviewPendingPanelEl.hidden = true;
      } else if (btn.dataset.tab === 'collect') {
        viewMode = 'collect';
...
if (harvestPanelEl) {
  ...
  harvestPanelEl.querySelector('.btn-review-automation-save')?.addEventListener('click', async () => {
    ...
    const body = {
      enabled: !!harvestPanelEl.querySelector('[data-ra="reviewAutomationEnabled"]')?.checked,
      targetScore: ts,
...
reviewPendingPanelEl?.querySelector('.btn-review-automation-save-pending')?.addEventListener('click', async () => {
  ...
  const body = {
    enabled: !!reviewPendingPanelEl.querySelector('[data-ra="reviewAutomationEnabled"]')?.checked,
    targetScore: ts,
...
// Инициализация роутинга: применить текущий URL и загрузить соответствующую страницу
applyRouteFromUrl();
refreshHarvestStats().catch(() => {});
initDraftModalDrag();
...
function initScrollToTop() {
  ...
  function updateVisibility() {
    ...
    const shouldHide = scrollY <= HIDE_THRESHOLD;

    if (shouldShow && !btn.classList.contains('visible')) {
      btn.classList.add('visible');
      btn.hidden = false;
    } else if (shouldHide && btn.classList.contains('visible')) {
      btn.classList.remove('visible');
      // Delay hiding attribute to allow transition to complete
      setTimeout(() => {
...
/** Custom number input spinners click and hover handling */
function initCustomNumberSpinners() {
  // Track current hovered input for efficient class cleanup
  let currentHoveredInput = null;

  // Handle clicks on number inputs with custom spinners
  document.addEventListener('click', (e) => {
    const input = e.target.closest('.harvest-field input[type="number"], .review-field input[type="number"]');
    if (!input) return;
    ...
    // Check if click is in the spinner area (right side, ~18px width)
    if (x < rect.width - 18) return;
  ...
  // Handle hover states for individual arrows
  document.addEventListener('mousemove', (e) => {
    const input = e.target.closest('.harvest-field input[type="number"], .review-field input[type="number"]');

    // Clear classes from previous input if mouse left it
    if (currentHoveredInput && currentHoveredInput !== input) {
      currentHoveredInput.classList.remove('arrow-up-hover', 'arrow-down-hover');
      currentHoveredInput.style.cursor = '';
    ...
    // Check if in spinner area (right side, ~18px width)
    const inSpinnerArea = x >= rect.width - 18;

    if (!inSpinnerArea) {
      // In input but not in spinner area - clear all hover classes
      input.classList.remove('arrow-up-hover', 'arrow-down-hover');
      input.style.cursor = '';
      return;
    }

    // In spinner area - determine which half
    ...
    if (isTopHalf) {
      input.classList.add('arrow-up-hover');
      input.classList.remove('arrow-down-hover');
    } else {
      input.classList.add('arrow-down-hover');
      input.classList.remove('arrow-up-hover');
    }
  });

  // Clean up classes when mouse leaves the document or input area
  document.addEventListener('mouseout', (e) => {
    const input = e.target.closest('.harvest-field input[type="number"], .review-field input[type="number"]');
    if (input) {
      input.classList.remove('arrow-up-hover', 'arrow-down-hover');
      input.style.cursor = '';
      if (currentHoveredInput === input) {
        currentHoveredInput = null;
      }
    }
  });
