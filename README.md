...
- **Сбор вакансий** — вкладка «Сбор вакансий»: параметры `HH_*` (лимиты, паузы, джиттеры, логика ключей, опционально рабочие часы) подставляются из `.env`, при изменении в форме уходят в фоновый `harvest` и **перекрывают** `.env`. Пайплайн как у `npm run harvest` (поиск по `config/search-keywords.txt`, парсинг, три скора LLM). Режим ключей: **зациклить** (бесконечные проходы до остановки процесса), **N циклов** (полный список ключей за цикл, повторить N раз), **K ключей** (один проход только по первым K строкам файла). `HH_WORK_HOURS_ENABLED=1` — вне интервала `HH_WORK_HOUR_START`–`HH_WORK_HOUR_END` (часы суток, включительно, локальное время) скрипт ждёт без поиска и открытия карточек. После каждого завершённого прохода первая строка ключей **ротируется в конец** (отключить: `HH_ROTATE_KEYWORD_AFTER_RUN=0`). Лог: `data/harvest-run.log`. Одновременно допускается только один такой процесс.
